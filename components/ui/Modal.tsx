...
export function Modal({ isOpen, onClose, title, children, size = 'md' }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div
        className={[
          'border-border relative z-10 w-full rounded-[var(--radius-xl)] border',
      ...
      >
        ...
        <div className="border-border flex items-center justify-between border-b px-6 py-4">
          ...
          <button
            onClick={onClose}
            className="text-text-secondary hover:bg-surface-alt hover:text-text-primary rounded-[var(--radius-sm)] p-1"
            aria-label="Close"
          >
            ✕
          </button>
